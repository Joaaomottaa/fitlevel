-- ============================================================
-- FitLevel - Saúde Gamificada · Schema Supabase
-- Cole este arquivo inteiro no SQL Editor do Supabase e execute.
-- (é idempotente - pode rodar de novo em um banco já criado)
-- ============================================================

-- Perfil público do jogador (alimenta ranking global e busca por código)
...
-- ============================================================
-- Código de amizade: identifica o usuário no "adicionar amigo".
-- Migração segura para bancos que já rodaram a versão anterior.
-- ============================================================
alter table public.profiles add column if not exists codigo_amigo text;

update public.profiles
  set codigo_amigo = 'FL-' || upper(substr(replace(id::text, '-', ''), 1, 6))
  where codigo_amigo is null;

create unique index if not exists idx_profiles_codigo on public.profiles (codigo_amigo);
...
begin
  insert into public.profiles (id, nome, codigo_amigo)
  values (
    new.id,
    coalesce(new.raw_user_meta_data ->> 'nome', ''),
    'FL-' || upper(substr(replace(new.id::text, '-', ''), 1, 6))
  )
